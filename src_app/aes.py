...
def aes_labelize(plaintexts, keys, target_byte, leakage_model="ID"):
    ...
    return np.array([bin(iv).count("1") for iv in intermediates]) if leakage_model == "HW" else intermediates


def create_intermediates(profiling_plaintext, profiling_masks, profiling_key, n_p,
                         attack_plaintext=None, attack_key=None, attack_masks=None, n_a=None,
                         leakage_model="ID", dataset=""):
    ...
    if dataset == "ascadf":
        ...
        if n_a is not None:
            share2_attack[0, :] = [AES_Sbox[int(p) ^ int(k)] for p, k in
        ...
        for byte in range(16):
            ...
            if n_a is not None:
                ...
                share2_attack[byte, :] = [AES_Sbox[int(p) ^ int(k)] ^ int(r) for p, k, r in
                                          zip(np.asarray(attack_plaintext[:, byte]),
                                              np.asarray(attack_key[:, byte]),
    ...
    return share1_profiling, share2_profiling
